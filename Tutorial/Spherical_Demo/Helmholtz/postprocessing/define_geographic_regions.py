...
regions = ['Global', 'Tropics', 'North of Tropics', 'South of Tropics',
           '25th KE Percentile', '50th KE Percentile', '75th KE Percentile']
...
with Dataset('region_definitions.nc', 'w', format='NETCDF4') as fp:
    ...
    # We can also define some more detailed regions. In this case, by KE
    with Dataset(vel_file, 'r') as dset:
        KE = dset['uo'][0,0,:,:]**2 + dset['vo'][0,0,:,:]**2
    percentiles = np.percentile( KE, [25,50,75] )

    reg_var[4,:,:] = KE > percentiles[0];
    reg_var[5,:,:] = KE > percentiles[1];
    reg_var[6,:,:] = KE > percentiles[2];
