...
void compute_coarsened_map(
    ...
    for (Ifield = 0; Ifield < num_fields; ++Ifield) {
        ...
        private(Ilat, Ilon, index, coarse_index, dA, dA_coarse, \
                ...
                for (Idepth = 0; Idepth < Ndepth; ++Idepth) {
                    for (Ilat_coarse = 0; Ilat_coarse < Nlat_coarse; ++Ilat_coarse) {
                        for (Ilon_coarse = 0; Ilon_coarse < Nlon_coarse; ++Ilon_coarse) {
                            ...
                            coarse_index = Index(0, 0, Ilat_coarse, Ilon_coarse, Ntime, Ndepth, Nlat_coarse, Nlon_coarse);
                            dA = source_data.coarse_map_areas.at(coarse_index);
