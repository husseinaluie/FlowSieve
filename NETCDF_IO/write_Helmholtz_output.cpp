...
void write_Helmholtz_output(
    ...
    std::vector<std::string> vars = {"uo", "vo", "Psi", "Phi", "projected_uo", "projected_vo", 
                    "projected_uo_tor", "projected_uo_pot", "projected_vo_tor", "projected_vo_pot",
                    "vorticity", "divergence", "projected_vorticity", "projected_divergence",
                    "vort_of_proj_vel", "div_of_proj_vel"
    };
    if (wRank == 0) {
    ...
    write_field_to_output( source_data.variables.at("u_lon"), "uo", starts, counts, filename, &source_data.mask);
    write_field_to_output( source_data.variables.at("u_lat"), "vo", starts, counts, filename, &source_data.mask);

    write_field_to_output( source_data.variables.at("vort"), "vorticity", starts, counts, filename, &source_data.mask);
    write_field_to_output( source_data.variables.at("div"), "divergence", starts, counts, filename, &source_data.mask);

    write_field_to_output( solution_data.variables.at("Psi"), "Psi", starts, counts, filename );
    write_field_to_output( solution_data.variables.at("Phi"), "Phi", starts, counts, filename );

    write_field_to_output( source_data.variables.at("projected_u_lon"), "projected_uo", starts, counts, filename );
    write_field_to_output( source_data.variables.at("projected_u_lat"), "projected_vo", starts, counts, filename );

    write_field_to_output( source_data.variables.at("proj_uo_tor"), "projected_uo_tor", starts, counts, filename );
    write_field_to_output( source_data.variables.at("proj_vo_tor"), "projected_vo_tor", starts, counts, filename );
    write_field_to_output( source_data.variables.at("proj_uo_pot"), "projected_uo_pot", starts, counts, filename );
    write_field_to_output( source_data.variables.at("proj_vo_pot"), "projected_vo_pot", starts, counts, filename );

    write_field_to_output( source_data.variables.at("projected_vort"), "projected_vorticity", starts, counts, filename );
    write_field_to_output( source_data.variables.at("projected_div" ), "projected_divergence",  starts, counts, filename );

    write_field_to_output( source_data.variables.at("proj_vort"), "vort_of_proj_vel", starts, counts, filename );
    write_field_to_output( source_data.variables.at("proj_div" ), "div_of_proj_vel",  starts, counts, filename );


    //
    //// Add and write convergence metrics
    //
    vars = {"vel_2_errors",  "vel_inf_errors",  "vel_2_norms",  "vel_inf_norms", 
            "vort_2_errors", "vort_inf_errors", "vort_2_norms", "vort_inf_norms", 
            "div_2_errors",  "div_inf_errors",  "div_2_norms",  "div_inf_norms"
    };
    if (wRank == 0) {
        // Loop through and add the desired variables
        // Dimension names (in order!)
        const char* dim_names[] = {"time", "depth", "IterCycle"};
        const int ndims = 3;
        for (size_t varInd = 0; varInd < vars.size(); ++varInd) {
    ...
    size_t err_starts[3] = { size_t(myStarts.at(0)), size_t(myStarts.at(1)), 0 };
    size_t err_counts[3] = { size_t(myCounts.at(0)), size_t(myCounts.at(1)), Helmholtz_data.vel_2_errors.size() };
    write_field_to_output( Helmholtz_data.vel_2_errors, "vel_2_errors", err_starts, err_counts, filename);
    write_field_to_output( Helmholtz_data.vel_2_norms,  "vel_2_norms",  err_starts, err_counts, filename);
    write_field_to_output( Helmholtz_data.vel_inf_errors, "vel_inf_errors", err_starts, err_counts, filename);
    write_field_to_output( Helmholtz_data.vel_inf_norms,  "vel_inf_norms",  err_starts, err_counts, filename);

    write_field_to_output( Helmholtz_data.vort_2_errors, "vort_2_errors", err_starts, err_counts, filename);
    write_field_to_output( Helmholtz_data.vort_2_norms,  "vort_2_norms",  err_starts, err_counts, filename);
    write_field_to_output( Helmholtz_data.vort_inf_errors, "vort_inf_errors", err_starts, err_counts, filename);
    write_field_to_output( Helmholtz_data.vort_inf_norms,  "vort_inf_norms",  err_starts, err_counts, filename);

    write_field_to_output( Helmholtz_data.div_2_errors, "div_2_errors", err_starts, err_counts, filename);
    write_field_to_output( Helmholtz_data.div_2_norms,  "div_2_norms",  err_starts, err_counts, filename);
    write_field_to_output( Helmholtz_data.div_inf_errors, "div_inf_errors", err_starts, err_counts, filename);
    write_field_to_output( Helmholtz_data.div_inf_norms,  "div_inf_norms",  err_starts, err_counts, filename);
