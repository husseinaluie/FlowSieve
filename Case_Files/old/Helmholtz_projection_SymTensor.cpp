...
int main(int argc, char *argv[]) {
    ...
    const std::string   &input_fname        = input.getCmdOption("--input_file",    "input.nc"),
                        &output_fname       = input.getCmdOption("--output_file",   "projection_Helmholtz_SymTensor.nc"),
    ...
    const std::string   &zonal_vel_name    = input.getCmdOption("--zonal_vel",   "uo"),
                        ...
                        &v_lon_seed_name   = input.getCmdOption("--v_lon_seed",  "v_lon_seed"),
                        &v_lat_seed_name   = input.getCmdOption("--v_lat_seed",  "v_lat_seed");
    ...
    double seed_count;
    bool single_seed;
    std::vector<double> seed_v_r, seed_v_lon, seed_v_lat;
    if (seed_fname == "zero") {
        seed_count = 1.;
        seed_v_r.resize(   source_data.Nlat * source_data.Nlon, 0. );
        seed_v_lon.resize( source_data.Nlat * source_data.Nlon, 0. );
        seed_v_lat.resize( source_data.Nlat * source_data.Nlon, 0. );
    } else {
        ...
        read_var_from_file( seed_v_lon, v_lon_seed_name, seed_fname, NULL, NULL, NULL, Nprocs_in_time, Nprocs_in_depth, not(single_seed) );
        read_var_from_file( seed_v_lat, v_lat_seed_name, seed_fname, NULL, NULL, NULL, Nprocs_in_time, Nprocs_in_depth, not(single_seed) );
    }
    single_seed = (seed_count == 1);

    // Apply to projection routine
    Apply_Helmholtz_Projection_SymTensor( output_fname, source_data, seed_v_r, seed_v_lon, seed_v_lat, 
            single_seed, tolerance, max_iterations, use_area_weight, use_mask );
