...
int main(int argc, char *argv[]) {
    ...
    const std::string   &scalar_var_name    = input.getCmdOption("--scalar",   
                                                                "scalar",          
                                                                asked_help,
                                                                "Name of scalar in netCDF input file"),
    ...
    const int num_refinements = stod(num_refine_string);  
    ...
    if (not(constants::GRID_TYPE == constants::GridType::MeshGrid)) {
        source_data.Nlat = 1;
        source_data.Nlon = 1;
    }
    ...
    // Read in the scalar
    source_data.load_variable( "scalar", scalar_var_name, input_fname, true, true );
    
    // If requested, extend to poles and pad with land
    if (constants::EXTEND_DOMAIN_TO_POLES) {
        if (constants::GRID_TYPE != constants::GridType::MeshGrid) {
            throw std::logic_error("Cannot extend domain to poles when not using a mesh grid.");
        }
        ...
        // Extend scalar field
        extend_field_to_poles( source_data.variables.at("scalar"), source_data, extended_latitude, orig_lat_start_in_extend );

        // Update the latitude vector and size variables
    ...
    fprintf( stdout, "Getting cell areas.\n" );
    if (constants::GRID_TYPE == constants::GridType::MeshGrid) {
    ...
    fprintf( stdout, "Beginning solver.\n" );
    Scalar_Solver( "output.nc", source_data, tolerance, max_iterations, iters_per_cycle, use_area_weight,
           use_mask, num_refinements );
