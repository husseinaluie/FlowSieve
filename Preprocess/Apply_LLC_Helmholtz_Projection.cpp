...
void Apply_LLC_Helmholtz_Projection(
    ...
    for ( size_t Ipt = 0; Ipt < Npts; Ipt++ ) {
        ...
        double weight_val = 1.; // just hard code for now
        double cos_lat_inv = 1. / cos(latitude.at(Ipt)),
        ...
        for ( size_t Ineighbour = 0; Ineighbour < num_neighbours + 1; Ineighbour++ ) {
            ...
            bool is_pole = std::fabs( std::fabs( latitude.at(Ipt) * 180.0 / M_PI ) - 90 ) < 0.01;
            if ( is_pole ) { continue; }
        ...
        for (int Idepth = 0; Idepth < Ndepth; ++Idepth) {
            ...
            firstprivate( Ntime, Ndepth )
            {
                ...
                for (index_sub = 0; index_sub < Npts; ++index_sub) {
                    ...
                    //u_lon_rem.at( index_sub ) = u_lon.at(index);
                    //u_lat_rem.at( index_sub ) = u_lat.at(index);
                }
            }
            ...
            firstprivate( Ndepth, Ntime, Npts, Tikhov_Laplace, weight_err, stdout )
            {
                ...
                for (index_sub = 0; index_sub < Npts; ++index_sub) {
                    ...
                    is_pole = std::fabs( std::fabs( latitude.at(index_sub) * 180.0 / M_PI ) - 90 ) < 0.01;
