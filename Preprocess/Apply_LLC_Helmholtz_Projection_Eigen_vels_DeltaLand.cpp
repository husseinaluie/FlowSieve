...
void Apply_LLC_Helmholtz_Projection_Eigen_vels_DeltaLand(
    ...
    std::vector<T> Aij_triplets( 4 * Nrow * pts_per_1st_deriv + 2, T(0,0,0) );
    Aij_triplets[Aij_triplets.size() - 2] = T( 2 * Nrow + 0, 0,    1 ); // Force Psi = 0 on first point
    Aij_triplets[Aij_triplets.size() - 1] = T( 2 * Nrow + 1, Ncol, 1 ); // Force Phi = 0 on first point
    ...
    private( Ipt, Ineighbour, neighbour_ind, Itriplet, row_skip, column_skip, is_pole, val, \
             weight_val, cos_lat_inv, R_inv, rand_val, \
    ...
    Eigen::SparseMatrix<double> LHS_matr( 2*Nrow+2, 2*Ncol );
    ...
    for (int Itime = 0; Itime < Ntime; ++Itime) {
        for (int Idepth = 0; Idepth < Ndepth; ++Idepth) {
            ...
            private( Ipt ) firstprivate( weight_err, Npts, Nrow )
            {
                #pragma omp for collapse(1) schedule(static)
                for ( Ipt = 0; Ipt < Npts; ++Ipt) {
