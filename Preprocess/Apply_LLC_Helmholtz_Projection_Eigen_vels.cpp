...
void Apply_LLC_Helmholtz_Projection_Eigen_vels(
    ...
    size_t num_land_points = 0, Ineighbour, Ipt, neighbour_ind;
    bool all_land_neighbours;
    if (constants::FILTER_OVER_LAND) {
        for (Ipt = 0; Ipt < u_lon.size(); Ipt++) {
            all_land_neighbours = not(mask[Ipt]);
            ...
            if ( all_land_neighbours ) { num_land_points++; }
        }
    }
    #if DEBUG>=0
    ...
    // alglib variables
    std::vector<double> 
        RHS_vector( 2*Npts + 2*constants::ADJACENCY_SIZE*num_land_points, 0. ),
        Psi_seed(       Npts, 0. ),
        Phi_seed(       Npts, 0. ),
        work_arr(       Npts, 0. ),
        div_term(       Npts, 0. ),
    ...
    std::vector<T> Aij_triplets( 4 * Npts * pts_per_1st_deriv + 4 * num_land_points * constants::ADJACENCY_SIZE, T(0,0,0) );
    size_t Itriplet;
    ...
    private( Ipt, Ineighbour, neighbour_ind, Itriplet, row_skip, column_skip, is_pole, val, \
             weight_val, cos_lat_inv, R_inv, all_land_neighbours, rand_val ) \
    ...
    { 
        ...
        for ( Ipt = 0; Ipt < Npts; Ipt++ ) {
            ...
            if ( not(mask[Ipt]) and constants::FILTER_OVER_LAND ) {
                ...
                if ( all_land_neighbours ) {
                    ...
                    row_skip    = Ipt;
                    Itriplet    = Ipt * pts_per_1st_deriv + 0;
                    ...
                    row_skip    = Npts + Ipt;
                    Itriplet    = (2*Npts + Ipt) * pts_per_1st_deriv + 0;
                    Aij_triplets[Itriplet  ] = T( row_skip, Npts + Ipt,            weight_val * Tikhov_Laplace );
                    Aij_triplets[Itriplet+1] = T( row_skip, Npts + neighbour_ind, -weight_val * Tikhov_Laplace );
                    continue;
                }
            }
            ...
            for ( Ineighbour = 0; Ineighbour < num_neighbours + 1; Ineighbour++ ) {
                ...
                // Psi part (of u_lat)
                //if ( (mask[Ipt]) or (Ipt % 2 == 0) ) {
                    column_skip = 0 * Npts + neighbour_ind;
                    row_skip    = 1 * Npts + Ipt;
                    Itriplet = Ipt * pts_per_1st_deriv + Ineighbour;
                ...
                //if ( (mask[Ipt]) or (Ipt % 2 == 0) ) {
                    ...
                    row_skip    = 0 * Npts + Ipt;
                    Itriplet = (2*Npts+Ipt) * pts_per_1st_deriv + Ineighbour;
                    Aij_triplets[Itriplet] = T( row_skip, column_skip, val );
                //}
                ...
                // Psi part (of u_lon)
                //if ( (mask[Ipt]) or (Ipt % 2 == 1) ) {
                    column_skip = 0 * Npts + neighbour_ind;
                    row_skip    = 0 * Npts + Ipt;
                    Itriplet = (Npts + Ipt) * pts_per_1st_deriv + Ineighbour;
                    Aij_triplets[Itriplet] = T( row_skip, column_skip, -val );
                //}
                ...
                //if ( (mask[Ipt]) or (Ipt % 2 == 1) ) {
                    column_skip = 1 * Npts + neighbour_ind;
                    row_skip    = 1 * Npts + Ipt;
                    Itriplet = (3*Npts + Ipt) * pts_per_1st_deriv + Ineighbour;
                    Aij_triplets[Itriplet] = T( row_skip, column_skip, val );
                //}
    ...
    if ( (num_land_points > 0) and (wRank == 0) ) {
        fprintf( stdout, "Adding land-fixing terms to LHS\n" );
    }
                ...
                if ( not(mask[Ipt]) ) {
                    if ( Iland == count ) {
                        ...
                        for ( Ineighbour = 0; Ineighbour < num_neighbours; Ineighbour++ ) {
                            ...
                            row = 2 * Npts + Iland * constants::ADJACENCY_SIZE + Ineighbour;
                            Itriplet = Npts * (4 * pts_per_1st_deriv)  +  2 * (Iland * constants::ADJACENCY_SIZE + Ineighbour);
                            ...
                            row = 2 * Npts + (Iland + num_land_points) * constants::ADJACENCY_SIZE + Ineighbour;
                            Itriplet = Npts * (4 * pts_per_1st_deriv)  +  2 * ((Iland+num_land_points) * constants::ADJACENCY_SIZE + Ineighbour);
                            Aij_triplets[Itriplet  ] = T( row, Npts + Ipt,            weight_val * Tikhov_Laplace );
                            Aij_triplets[Itriplet+1] = T( row, Npts + neighbour_ind, -weight_val * Tikhov_Laplace );
                        }
                    }
                    count++;
                }
    ...
    Eigen::SparseMatrix<double> LHS_matr( 2*Npts + 2*constants::ADJACENCY_SIZE*num_land_points, 2*Npts );
    fprintf( stdout, "%'zu, %'zu\n", 2*Npts + 2*constants::ADJACENCY_SIZE*num_land_points, 2*Npts );
    ...
    for (int Itime = 0; Itime < Ntime; ++Itime) {
        for (int Idepth = 0; Idepth < Ndepth; ++Idepth) {
            ...
            #pragma omp parallel default(none) \
            shared( dAreas, RHS_vector, u_lon_rem, u_lat_rem ) \
            private( index_sub ) firstprivate( weight_err, Npts )
            {
                #pragma omp for collapse(1) schedule(static)
                for (index_sub = 0; index_sub < Npts; ++index_sub) {
                    RHS_vector.at(       index_sub) = u_lon_rem.at(index_sub) * ( weight_err ? dAreas.at(index_sub) : 1. );
                    RHS_vector.at(Npts + index_sub) = u_lat_rem.at(index_sub) * ( weight_err ? dAreas.at(index_sub) : 1. );
                }
            }
