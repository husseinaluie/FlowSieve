...
void Apply_LLC_Helmholtz_Projection_Eigen_both(
    ...
    std::vector<double> 
        RHS_vector( 4 * Npts + constants::ADJACENCY_SIZE*num_land_points, 0. ),
        Psi_seed(       Npts, 0. ),
        Phi_seed(       Npts, 0. ),
        work_arr(       Npts, 0. ),
        div_term(       Npts, 0. ),
    ...
    private( Ipt, Ineighbour, neighbour_ind, Itriplet, row_skip, column_skip, is_pole, val, \
             weight_val, cos_lat_inv, R_inv, all_land_neighbours, counter ) \
            ...
            for ( Ineighbour = 0; Ineighbour < num_neighbours + 1; Ineighbour++ ) {
                ...
                for ( counter = 0; counter < 2; counter++ ) {
                    ...
                    if ( USE_TRUE_2ND_DERIV ) {
                        ...
                        val *= 1. / deriv_scale_factor;

                        column_skip = neighbour_ind;
                        column_skip += counter * Npts;
                        row_skip    = Ipt;
                        row_skip    += 2 * Npts;
                        row_skip    += counter * Npts;
                        Itriplet = Ipt * pts_per_2nd_deriv + Ineighbour;
                        Itriplet += 4 * Npts * pts_per_1st_deriv;
                        Aij_triplets[Itriplet] = T( row_skip, column_skip, val );
                    } else {
    ...
    Eigen::SparseMatrix<double> LHS_matr( 4 * Npts + constants::ADJACENCY_SIZE*num_land_points, 
            2 * Npts );
    ...
    for (int Itime = 0; Itime < Ntime; ++Itime) {
        for (int Idepth = 0; Idepth < Ndepth; ++Idepth) {
            ...
            #pragma omp parallel default(none) \
            shared( dAreas, RHS_vector, vort_term, div_term, u_lon_rem, u_lat_rem ) \
            private( index_sub ) firstprivate( weight_err, Npts )
            {
                #pragma omp for collapse(1) schedule(static)
                for (index_sub = 0; index_sub < Npts; ++index_sub) {
                    RHS_vector.at(0*Npts + index_sub) = u_lon_rem.at(index_sub) * ( weight_err ? dAreas.at(index_sub) : 1. );
                    RHS_vector.at(1*Npts + index_sub) = u_lat_rem.at(index_sub) * ( weight_err ? dAreas.at(index_sub) : 1. );
                    RHS_vector.at(2*Npts + index_sub) = vort_term.at(index_sub) * ( weight_err ? dAreas.at(index_sub) : 1. );
                    RHS_vector.at(3*Npts + index_sub) = div_term.at(index_sub) * ( weight_err ? dAreas.at(index_sub) : 1. );
                }
            }
